...
from app.models import Inspection, Issue, Restroom, UtilityReading
...
def delete_restroom(db: Session, restroom_id: int, *, force: bool = False) -> None:
    ...
    utility_count = db.scalar(
        select(func.count()).select_from(UtilityReading).where(UtilityReading.restroom_id == restroom_id)
    ) or 0
    if (inspection_count or issue_count or utility_count) and not force:
        raise ConflictError(
            f"该公厕已有 {inspection_count} 条巡查记录、{issue_count} 条问题记录、"
            f"{utility_count} 条抄表记录，确需删除请使用 force=true"
        )
    db.delete(restroom)
    db.commit()
