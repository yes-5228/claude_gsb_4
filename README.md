...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 水电计量** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 水电计量 | `/utilities` | 按月抄录水表电表读数、自动核算用量与费用、与上月环比、超阈值标记用量偏高/偏低并提示可能原因、手工费用、历史月份修改后链式重算 |
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / issues / utility-records / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、水电抄表
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、评分、统计、水电计量
│   │   ├── seed.py              # 演示数据生成
...
| `WATER_UNIT_PRICE` | `4.6` | 水费默认单价（元/吨），建账时快照到记录 |
| `ELEC_UNIT_PRICE` | `0.8` | 电费默认单价（元/度），建账时快照到记录 |
...
| GET | `/utility-records` | 水电月度台账（公厕/区域/年/月/异常/抄表人筛选、分页） |
| POST | `/utility-records` | 登记月度水电抄表，自动重算用量、费用、环比与异常 |
| GET | `/utility-records/baseline` | 新增抄表时的上期表底/用量与当前单价（表单实时预览用） |
| GET/PATCH/DELETE | `/utility-records/{id}` | 详情 / 更新（账期不可改）/ 删除（后续月份自动重算） |
...
- **水电月度计量**：一座公厕一个自然月仅一条记录（水、电同条）。用量 = 本期表底 − 上一条记录表底（按账期向前顺延，支持只抄水或只抄电、跨月补录）；费用默认 = 用量 × 建账时单价快照，也可改为手工填写（重算时保留）。
- **水电环比与异常**：以最近一个有效用量为上月基准计算环比增减量与百分比；环比超过 ±30% 且绝对变化量超过噪声门槛（水 2 吨、电 30 度）时分别标记「用量偏高/用量偏低」，本期表底低于上期（换表归零/抄录错误）单独按表底回退提示；每种异常都附可能原因。首个可计量月份、上月用量为 0 时不计算百分比。任何新增、修改（含改历史月份）、删除都会按时间顺序重算该公厕后续月份。
- **删除保护**：删除公厕时若已存在巡查、问题或水电记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、近 8 个月 9 座开放公厕的 72 条水电抄表记录（累计表底随机游走，含用水/用电突增与水表回退等异常样例）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、流程流转、统计、水电计量环比与异常）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：20 个用例全部通过（含水电计量 14 个：用量/费用、环比、偏高/偏低/回退异常、噪声门槛、手工费用锁定、历史修改链式重算、删除重算、重复账期、级联删除、字典、跨年排序）；`/health`、台账/巡查/问题/水电/统计/字典接口均返回预期数据。
- 前端 `npm run build`：构建成功（73 个模块）。
- 水电模块接口冒烟：以临时 SQLite 库启动 uvicorn 并写入演示数据后，验证列表分页与公厕/区域/异常筛选、字典单价与原因、baseline 上期参考、新增（自动计费 + 手工费用锁定）、详情、重复账期 400、手工费用切回自动重算、删除 404 等链路均符合预期。
